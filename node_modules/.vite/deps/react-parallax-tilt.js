...
var n = (e, t, i) => Math.min(Math.max(e, t), i);
var s = class {
	glareWrapperEl;
	glareEl;
	glareAngle = 0;
	...
	constructor(e, t) {
		...
		Object.assign(this.glareWrapperEl.style, i), Object.assign(this.glareEl.style, s);
	}
	calculateGlareSize = (e) => {
		const { width: t, height: i } = e, n = Math.sqrt(t ** 2 + i ** 2);
		return {
			width: n,
			height: n
		};
	...
	updateAngle = (e, t) => {
		const { xPercentage: i, yPercentage: n } = e, s = 180 / Math.PI, r = i ? Math.atan2(n, -i) * s : 0;
		this.glareAngle = r - (t ? 180 : 0);
	};
	updateOpacity = (e, t, i, s) => {
		const { xPercentage: r, yPercentage: l } = e, { glarePosition: a, glareReverse: o, glareMaxOpacity: p } = t, h = i ? -1 : 1, c = s ? -1 : 1, g = o ? -1 : 1;
		let d = 0;
		switch (a) {
			case "top":
				d = -r * h * g;
...
var r = class {
	...
	updateTilt = (e, t) => {
		const { xPercentage: i, yPercentage: n } = e, { tiltMaxAngleX: s, tiltMaxAngleY: r } = t;
		this.tiltAngleX = i * s / 100, this.tiltAngleY = n * r / 100 * -1;
	};
	updateTiltManualInput = (e, t) => {
		const { tiltAngleXManual: i, tiltAngleYManual: n, tiltMaxAngleX: s, tiltMaxAngleY: r } = t;
	...
	updateTiltLimits = (e) => {
		...
		t && (this.tiltAngleX = "x" === t ? this.tiltAngleX : 0, this.tiltAngleY = "y" === t ? this.tiltAngleY : 0);
	};
	updateTiltAnglesPercentage = (e) => {
...
var a = class extends import_react.PureComponent {
	...
	addDeviceOrientationEventListener = async () => {
		if (!window.DeviceOrientationEvent) return;
		const e = DeviceOrientationEvent.requestPermission;
		if ("function" == typeof e) {
			"granted" === await e() && window.addEventListener("deviceorientation", this.onMove);
			return;
		}
	...
	onEnter = (e) => {
		...
		this.setSize(), this.wrapperEl.node.style.willChange = "transform", this.setTransitions(), t && t({ event: e });
	};
	onMove = (e) => {
		this.mainLoop(e), this.emitOnMove(e);
	};
	...
	onLeave = (e) => {
		...
		if (this.setTransitions(), t && t({ event: e }), this.props.reset) {
			const e = new CustomEvent("autoreset");
			this.onMove(e);
		}
	};
	processInput = (e) => {
		...
		switch (e.type) {
			...
			case "deviceorientation":
				...
				break;
			case "autoreset": {
				const { tiltAngleXInitial: e, tiltAngleYInitial: t, tiltMaxAngleX: i, tiltMaxAngleY: s } = this.props, r = t / s * 100;
				this.wrapperEl.clientPosition.xPercentage = n(e / i * 100, -100, 100), this.wrapperEl.clientPosition.yPercentage = n(r, -100, 100), this.wrapperEl.scale = 1;
				break;
			}
		}
	};
	processInputDeviceOrientation = (e) => {
		if (!e.gamma || !e.beta || !this.props.gyroscope) return;
		const { tiltMaxAngleX: t, tiltMaxAngleY: i } = this.props, s = e.gamma;
		this.wrapperEl.clientPosition.xPercentage = e.beta / t * 100, this.wrapperEl.clientPosition.yPercentage = s / i * 100, this.wrapperEl.clientPosition.xPercentage = n(this.wrapperEl.clientPosition.xPercentage, -100, 100), this.wrapperEl.clientPosition.yPercentage = n(this.wrapperEl.clientPosition.yPercentage, -100, 100);
	};
	update = (e) => {
		const { tiltEnable: t, flipVertically: i, flipHorizontally: n } = this.props;
	...
	updateClientInput = () => {
		...
		let t, i;
		if (e) {
			const { x: e, y: n } = this.wrapperEl.clientPosition;
			t = n / window.innerHeight * 200 - 100, i = e / window.innerWidth * 200 - 100;
		} else {
			const { size: { width: e, height: n, left: s, top: r }, clientPosition: { x: l, y: a } } = this.wrapperEl;
			t = (a - r) / n * 200 - 100, i = (l - s) / e * 200 - 100;
		}
		this.wrapperEl.clientPosition.xPercentage = n(t, -100, 100), this.wrapperEl.clientPosition.yPercentage = n(i, -100, 100);
	};
	updateFlip = () => {
	...
	render() {
		const { children: t, className: i, style: n } = this.props;
